  ...
  if (!clientEmail || !privateKey || !spreadsheetId) {
    if (req.method === 'GET') {
      return res.status(200).json({
        configured: false,
        message: 'Vercel environment variables GOOGLE_CLIENT_EMAIL, GOOGLE_PRIVATE_KEY, or GOOGLE_SHEET_ID not configured.',
      });
    }
  ...
  try {
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      if (action === 'saveAll' || Array.isArray(orders)) {
        const listToSave = Array.isArray(orders) ? orders : req.body.orders || [];
      ...
      // Fallback single order append
      const newOrderRow = Array.isArray(req.body) ? req.body : [
        req.body.id || req.body.quoteNumber || '',
